...
int process_data(dds::sub::DataReader< ::LampControl> reader)
{
    // Take all samples
    int count = 0;
    dds::sub::LoanedSamples< ::LampControl> samples = reader.take();
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::topic::Topic< ::LampControl> topic(participant, "Example LampControl");

    // Create a Subscriber and DataReader with default Qos
    dds::sub::Subscriber subscriber(participant);
    dds::sub::DataReader< ::LampControl> reader(subscriber, topic);
    ...
    while (!application::shutdown_requested && samples_read < sample_count) {
        std::cout << "::LampControl subscriber sleeping up to 1 sec..." << std::endl;
